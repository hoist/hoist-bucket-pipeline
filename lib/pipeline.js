...
Pipeline.prototype.add = function add() {
  ...
  if (key !== null ) {
    if(key.match(/^[0-9a-zA-Z_\-]{20}$/)) {
      return this.Model.Bucket.findOneAsync(key)
      .bind(this)
      .then(function (bucket) {
        if (bucket) {
          throw new HoistErrors.bucket.InvalidError('A bucket with id '+key+' already exists');
        }
        return this.addHelper(key, meta);

      });
    }
    return BBPromise.reject(new HoistErrors.bucket.InvalidError('Id must be an alphanumeric string 20 characters long'));
  } 
  key = null;
  return this.addHelper(key, meta);
};
